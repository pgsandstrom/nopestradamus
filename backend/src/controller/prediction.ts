 ...
 */
export const getCensoredPrediction = (prediction:Prediction, keepHashes:string[] = []):PredictionCensored => ({
  ...prediction,
  creater: {
    ...prediction.creater,
    hash: undefined,
...
export const getLatestPredictions = () =>
  queryString('SELECT title, body, hash from prediction where public is true ORDER BY created LIMIT 20').then(cursor => cursor.rows)
...
export const getPrediction = async (hash:string):Promise<Prediction> => {
  ...
  const creater:Creater= await query(SQL`SELECT hash, mail, accepted, accepted_mail_sent, end_mail_sent FROM creater WHERE prediction_hash = ${hash}`).then(cursor => ensureSingleGet(cursor))
  const participants:Participant[] = await query(SQL`SELECT hash, mail, accepted, accepted_mail_sent, end_mail_sent FROM participant WHERE prediction_hash = ${hash}`).then(cursor => cursor.rows)
...
export const getOldBetWithUnsentCreaterAcceptMails = () => queryString(`
SELECT DISTINCT prediction.hash FROM prediction
JOIN creater on prediction.hash = creater.prediction_hash
WHERE creater.accepted_mail_sent = false 
`).then(cursor => cursor.rows)

export const getOldBetWithUnsentAcceptMails = () => queryString(`
SELECT DISTINCT prediction.hash FROM prediction
...
export const getOldBetWithUnsentEndMails = () => queryString(`
SELECT DISTINCT prediction.hash FROM prediction
JOIN participant on prediction.hash = participant.prediction_hash
JOIN creater on prediction.hash = creater.prediction_hash
...
export const createPrediction = async (title:string, body:string, finishDate:string, isPublic:boolean, createrMail:string, participantList:string[]) => {
  const hash = uuid()
...
export const setCreaterAcceptMailSent = (hash:string) => query(SQL`UPDATE creater SET accepted_mail_sent = true where hash = ${hash}`)

export const setParticipantAcceptMailSent = (hash:string) => query(SQL`UPDATE participant SET accepted_mail_sent = true where hash = ${hash}`)

export const updateCreaterAcceptStatus = async (predictionHash:string, hash:string, accepted:boolean) => {
...
export const updateParticipantAcceptStatus = async (predictionHash:string, hash:string, accepted:boolean) => {
